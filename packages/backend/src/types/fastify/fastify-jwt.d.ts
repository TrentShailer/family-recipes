...
declare namespace fastifyJwt {
  ...
  export interface SignOptions
    extends Omit<SignerOptions, "expiresIn" | "notBefore"> {
  ...
  export interface JWT {
    ...
    verify<Decoded extends VerifyPayloadType>(
      token: string,
    ...
    decode<Decoded extends DecodePayloadType>(
      token: string,
    ...
    lookupToken(
      request: FastifyRequest,
      options?: FastifyJWTOptions["verify"]
    ): string;
  }
...
declare function fastifyJwt(
  ...params: Parameters<FastifyJwt>
): ReturnType<FastifyJwt>;
export = fastifyJwt;
